...
class UsuarioModel extends Model
{
    ...
    // Obtener usuarios activos para mensajería
    public function getUsuariosActivos()
    {
        $builder = $this->db->table('usuarios u');
        $builder->select('u.idusuario, u.nombreusuario, u.email, u.tipo_usuario, 
                         CONCAT(p.nombres, " ", p.apellidos) as nombres');
        $builder->join('personas p', 'u.idpersona = p.idpersona');
        $builder->where('u.estado', 1);
        $builder->where('u.idusuario !=', session('idusuario')); // Excluir usuario actual
        $builder->orderBy('p.nombres', 'ASC');
